  ...
  return (
    <div className="bg-white font-sans">
      ...
      <main className="p-4 space-y-6">
        <section>
          <div className="overflow-x-auto whitespace-nowrap flex space-x-4">
            {carousel.map(item => (
              <div
                key={item.id}
                className="flex-shrink-0 w-40 group cursor-pointer transition-all duration-200 hover:-translate-y-1 hover:scale-105"
                onClick={() => item.productUrl && window.open(item.productUrl, '_blank', 'noopener,noreferrer')}
              >
        ...
        <Button className="w-full" size="lg" onClick={refresh}>실데이터로 새로고침</Button>
