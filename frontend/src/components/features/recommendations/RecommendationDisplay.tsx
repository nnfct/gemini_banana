...
export const RecommendationDisplay: React.FC<RecommendationDisplayProps> = ({
        ...
        encodeURIComponent(
            `<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512">
                ...
                <g fill="#9ca3af" text-anchor="middle" font-family="Arial, Helvetica, sans-serif">
                    <text x="50%" y="50%" font-size="20" dy=".3em">?대?吏瑜?遺덈윭?????놁뒿?덈떎</text>
                </g>
            </svg>`
        );
                        ...
                        >
                            ...
                            <div className="space-y-1">
                                ...
                                {item.score !== undefined && (
                                    <p className="text-xs text-gray-500">?좎궗??{item.score}</p>
                                )}
                            </div>
                        </Card>
                    ))}
    ...
    if (!hasAnyRecommendations) {
        return (
            <Card className="text-center py-8">
                <h2 className="text-xl font-semibold text-gray-800 mb-2">異붿쿇 ?쒗뭹</h2>
                <p className="text-gray-600">異붿쿇???쒗뭹???놁뒿?덈떎.</p>
            </Card>
        );
    }
    ...
    return (
        <Card>
            <h2 className="text-2xl font-bold text-gray-800 mb-6">?좎궗 ?쒗뭹 異붿쿇</h2>
            <div>
